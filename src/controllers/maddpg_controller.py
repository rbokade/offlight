...
class MADDPGMAC:
    ...
    def get_repeat_actions(self, ep_batch, t, num_repeats=None):
        if num_repeats is not None:
            agent_inputs = self._build_inputs(ep_batch, t).reshape(
                ep_batch.batch_size, self.n_agents, -1
            )
            input_dim = agent_inputs.shape[-1]
            agent_inputs = (
                agent_inputs.unsqueeze(2)
                .repeat(1, 1, num_repeats, 1)
                .reshape(-1, input_dim)
            )
            avail_actions = (
                ep_batch["avail_actions"][:, t]
                .unsqueeze(2)
                .repeat(1, 1, num_repeats, 1)
            )
            ...
            agent_outs = agent_outs.view(
                ep_batch.batch_size, self.n_agents, num_repeats, -1
            )
        else:
    ...
    def init_hidden(self, batch_size, num_repeats=None):
        ...
        if num_repeats is not None:
            self.hidden_states = self.hidden_states.unsqueeze(2).repeat(
                1, 1, num_repeats, 1
            )
