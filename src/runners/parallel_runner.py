...
class ParallelRunner:
    ...
    def run(self, test_mode=False):
        ...
        while True:
            ...
            # Add local rewards
            post_transition_data.update({"local_rewards": []})
            post_transition_data.update({"network_flow": []})
            post_transition_data.update({"network_density": []})
            ...
            for idx, parent_conn in enumerate(self.parent_conns):
                if not terminated[idx]:
                    ...
                    post_transition_data["local_rewards"].append(data["local_rewards"])
                    post_transition_data["network_flow"].append((data["network_flow"],))
                    post_transition_data["network_flow"].append(
...
def env_worker(remote, env_fn):
    ...
    while True:
        ...
        if cmd == "step":
            ...
            local_rewards = env.get_local_rewards()
            # Return the observations, avail_actions and state to make the next action
            state = env.get_state()
            avail_actions = env.get_avail_actions()
            obs = env.get_obs()
            network_flow = env.get_network_flow()
            network_density = env.tsc_env.metrics.density
            step_data = {
            ...
            # Add local rewards
            try:
                step_data.update({"local_rewards": local_rewards})
            except Exception:
                pass
            remote.send(step_data)
        elif cmd == "reset":
            env.reset()
            reset_data = {
                "state": env.get_state(),
