...
class PPOLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if self.args.standardise_rewards:
            if self.args.standardise_rewards:
                rew_shape = (
                    (self.n_agents,) if args.use_local_rewards else (1,)
                )
    ...
    def train(
        ...
        actions = actions[:, :-1]
        if not self.args.use_local_rewards:
        ...
        for k in range(self.args.epochs):
            ...
            surr2 = (
                th.clamp(
                    ratios, 1 - self.args.eps_clip, 1 + self.args.eps_clip
                )
                * advantages
            )
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat(
                "pi_mean",
                (pi.mean(dim=-1)[0] * mask).sum().item() / mask.sum().item(),
                t_env,
            )
