...
class ActorCriticLearner:
    ...
    def train(
        self, batch: EpisodeBatch, t_env: int, episode_num: int, **kwargs
    ):
        ...
        if mask.sum() == 0:
            ...
            self.logger.console_logger.error(
                "Actor Critic Learner: mask.sum() == 0 at t_env {}".format(
                    t_env
                )
            )
            return
        ...
        pg_loss = (
            -(
                (advantages * log_pi_taken + self.args.entropy_coef * entropy)
                * mask
            ).sum()
            / mask.sum()
        )
    ...
    def train_critic_sequential(
        self, critic, target_critic, batch, rewards, mask
    ):
        # Optimise critic
        with th.no_grad():
        ...
        if self.args.standardise_returns:
            target_vals = (
                target_vals * th.sqrt(self.ret_ms.var) + self.ret_ms.mean
            )
        ...
        for t_start in range(rewards.size(1)):
            ...
            for step in range(nsteps + 1):
                ...
                if t >= rewards.size(1):
                    ...
                    t == rewards.size(1) - 1 and self.args.add_value_last_step
                ):
                    nstep_return_t += (
    ...
    def save_models(self, path):
        ...
        # th.save(
        #     self.agent_optimiser.state_dict(), "{}/agent_opt.th".format(path)
        # )
        # th.save(
        #     self.critic_optimiser.state_dict(), "{}/critic_opt.th".format(path)
        # )
    ...
    def load_models(self, path, transfer=False):
        ...
        self.critic.load_state_dict(
            th.load(
                "{}/critic.th".format(path),
                map_location=lambda storage, loc: storage,
            )
        )
        # Not quite right but I don't want to save target networks
        self.target_critic.load_state_dict(self.critic.state_dict())
        # if not transfer:  # if transferring, use a new optimizer
