...
class ICQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        q_vals = self.critic(critic_inputs).detach()[
            :, :-1
        ...
        actor_loss = (
            -(coe * (bs * advantages.detach() * log_pi_taken) * mask).sum() / mask.sum()
        )
    ...
    def train_critic(self, batch: EpisodeBatch):
        ...
        target_q_vals = self.target_critic(
            critic_inputs
        ).detach()  # (bs, seq_len, n_agents, n_action)

        with th.no_grad():
            # -----------------------------Q_lambda-IS-----------------------
            target_q_taken = th.gather(target_q_vals, dim=3, index=actions).squeeze(-1)
            target_q_vals_IS = self.target_mixer.icq_forward(
                target_q_taken, states
            )  # (bs, seq_len, 1)
            advantage_Q = F.softmax(
                target_q_vals_IS / self.icq_beta, dim=0
            )  # softmax over trajectory
            targets_taken = self.target_mixer.icq_forward(target_q_taken, states)
            targets_taken = (
                bs * advantage_Q * targets_taken
            )  # do not why there is a "bs*" operator
        ...
        for t in range(max_t - 1):
            ...
            critic_log["target_mean"].append(
                (target_q_t * mask_t).sum().item() / mask_elems
            )
            critic_log["q_taken_mean"].append(
        ...
        chosen_action_q_vals = self.mixer.icq_forward(
            chosen_action_q_vals, states[:, : max_t - 1]
        )
        # (bs, seq_len, 1)
        ...
        critic_log["target_mean"] = (
            target_q[:, filter_t] * mask[:, filter_t]
        ).sum().item() / mask_elems
        critic_log["q_taken_mean"] = (
            chosen_action_q_vals[:, filter_t] * mask[:, filter_t]
        ).sum().item() / mask_elems
        return critic_log
    ...
    def _build_critic_inputs(self, batch):
        ...
        inputs.append(
            batch["state"].unsqueeze(2).repeat(1, 1, self.n_agents, 1)
        )  # (bs, seq_len, n_agents, state_dim)
