...
class QLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if self.args.standardise_rewards:
            rew_shape = (self.n_agents,) if args.use_local_rewards else (1,)
            self.rew_ms = RunningMeanStd(shape=rew_shape, device=device)

        self.selected_agent = None
        if args.use_local_rewards:
            self.select_random_agent()

    def select_random_agent(self):
        self.selected_agent = random.randint(0, self.n_agents - 1)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, **kwargs):
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat(
                "td_error_max",
                (masked_td_error.max().item() / mask_elems),
                t_env,
            )
            self.logger.log_stat(
                "td_error_min",
                (masked_td_error.min().item() / mask_elems),
                t_env,
            )
            self.logger.log_stat(
                "q_taken_mean",
