...
class PACDCGLearner:
    ...
    def train_critic_sequential(self, batch, rewards, mask, terminated):
        ...
        if self.args.standardise_rewards:
            target_out = (
                target_out * th.sqrt(self.ret_ms.var) + self.ret_ms.mean
            )
            target_v = (
                target_v * th.sqrt(self.ret_ms_v.var) + self.ret_ms_v.mean
            )
        ...
        if self.args.standardise_rewards:
            ...
            target_returns_v = (
                target_returns_v - self.ret_ms_v.mean
            ) / th.sqrt(self.ret_ms_v.var)
        ...
        for t in range(batch.max_seq_length - 1):
            # In double Q-learning, the actions are selected greedy w.r.t. mac
            greedy_actions.append(
                self.critic.forward(batch, t=t, policy_mode=False)
            )
        greedy_actions = th.stack(greedy_actions, dim=1)
        max_q_a = []
        for i in range(self.n_agents):
