...
class DCGCriticNS:
    ...
    def annotations(self, ep_batch, t, compute_grads=False, actions=None):
        ...
        with th.no_grad() if not compute_grads else contextlib.suppress():
            ...
            for i, f in enumerate(self.utility_fun):
                f_i.append(
                    f(self.hidden_states[i]).reshape(ep_batch.batch_size, -1)
                )
    ...
    def q_values(self, f_i, f_ij, actions):
        ...
        if len(self.edges_from) > 0:
            ...
            edge_actions = actions.gather(
                dim=-2,
                index=self.edges_from.view(1, -1, 1).expand(n_batches, -1, 1),
            ) * self.n_actions + actions.gather(
                dim=-2,
                index=self.edges_to.view(1, -1, 1).expand(n_batches, -1, 1),
            )
    ...
    def _mlp(input, hidden_dims, output):
        ...
        hidden_dims = (
            [hidden_dims] if isinstance(hidden_dims, int) else hidden_dims
        )
        dim = input
        layers = []
    ...
    def forward(
        ...
        if policy_mode:  # ... either as policy tensor for the runner ...
            policy = f_i.new_zeros(
                ep_batch.batch_size, self.n_agents, self.n_actions
            )
            policy.scatter_(
                dim=-1,
                index=actions,
                src=policy.new_ones(1, 1, 1).expand_as(actions),
            )
            return policy
