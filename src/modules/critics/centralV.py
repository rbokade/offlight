...
#         h_in = h_in.expand(bs * self.n_agents, -1)
#         outputs = []
#         for i in range(inputs.shape[1]):
#             x = inputs[:, i].reshape(bs * self.n_agents, -1)
#             x = F.relu(self.fc1(x))
#             if self.args.use_rnn:
#                 h_in = self.rnn(x, h_in)
#             else:
#                 h_in = F.relu(self.rnn(x))
#             q = self.fc3(h_in)
#             outputs.append(q.view(bs, self.n_agents, 1))
...
#             elif isinstance(t, int):
#                 inputs.append(
#                     batch["actions_onehot"][:, slice(t - 1, t)].view(
#                         bs, max_t, 1, -1
#                     )
#                 )
#             else:
...
#         if self.args.obs_last_action:
#             input_shape += (
#                 scheme["actions_onehot"]["vshape"][0] * self.n_agents
#             )
#         input_shape += self.n_agents
#         return input_shape
